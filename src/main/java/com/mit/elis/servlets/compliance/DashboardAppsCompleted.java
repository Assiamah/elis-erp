...
@RestController
public class DashboardAppsCompleted {
	ws_compliance cls_compliance = new ws_compliance();
	@Autowired
	private Ws_url_config cls_url_config;

	@RequestMapping("/DashboardAppsCompleted")
	@PostMapping
	public String DashboardAppsCompleted(HttpSession session, Model model, HttpServletRequest request,
		...
		try {
			...
			if (requestType.equals("apps_completed_year")) {
				...
				web_service_response = cls_compliance
						.report_dashboard_completed_year_by_service_type_post(cls_url_config.getReport_web_service_url_ser(),
			...
			if (requestType.equals("apps_completed_year_applications")) {
				...
				web_service_response = cls_compliance
						.report_dashboard_completed_year_application_for_each_service_post(
