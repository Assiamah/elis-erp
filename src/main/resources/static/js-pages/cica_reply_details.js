document.addEventListener('DOMContentLoaded', function() {  
   ...
   $("#send_reply_to_client").on('submit', function(e) {
	...
	// Load status notes with Promise
	loadStatusNotes().then(() => {
		...
		Swal.fire({
			icon: 'error',
			title: 'Error',
			text: 'Failed to load status notes',
			timer: 2000,
			showConfirmButton: false
		});
	});
	
	// Load replies with Promise
	loadTicketReplies().then(() => {
		console.log('Ticket replies loaded');
	}).catch(error => {
		console.error('Error loading ticket replies:', error);
		Swal.fire({
			icon: 'error',
			title: 'Error',
			text: 'Failed to load ticket replies',
			timer: 2000,
			showConfirmButton: false
		});
	});
	
	// Load SMS messages with Promise
	loadSMSMessages().then(() => {
		console.log('SMS messages loaded');
	}).catch(error => {
		console.error('Error loading SMS messages:', error);
		Swal.fire({
			icon: 'error',
			title: 'Error',
			text: 'Failed to load SMS messages',
			timer: 2000,
			showConfirmButton: false
		});
	});
	
	// Function to load status notes
	function loadStatusNotes() {
			...
			$.ajax({
				...
				success: function(response) {
					...
					if(result.success == true) {
						if(result.data) {
							$("#notesCount").text(result.data.length)
						...
						reject(new Error('Failed to load status notes'));
					}
				},
	...
	// Function to load ticket replies
	function loadTicketReplies() {
		return new Promise((resolve, reject) => {
			...
			$.ajax({
				...
				success: function(response) {
					...
					if(result.success == true) {
						if(result.data) {
								$("#repliesCount").text(result.data.length)
							for(let i = 0; i < result.data.length; i++) {
								let html_ = [];
								num = (+num + 1);
						...
						reject(new Error('Failed to load ticket replies'));
					}
				},
				error: function(xhr, status, error) {
					reject(error);
		...
		return new Promise((resolve, reject) => {
			...
			$.ajax({
				...
				success: function(response) {
					...
					if(result.success == true) {
						if(result.data) {
							$("#smsCount").text(result.data.length)
							for(let i = 0; i < result.data.length; i++) {
								let html = [];
								num = (+num + 1);
						...
						reject(new Error('Failed to load SMS messages'));
					}
				},
				error: function(xhr, status, error) {
					reject(error);
	...
	$('#one_replies_tbl').on('click', '.link-post', function(event) {
