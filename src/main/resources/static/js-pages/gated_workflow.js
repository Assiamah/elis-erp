...
document.addEventListener('DOMContentLoaded', function() {
   ...
   $(document).on('click', '.newEncumberancesModalonMother', function (e) {
                        ...
                        <div class="d-flex">
                            ...
                            <div>
                                ...
                                <ul class="mb-0 ps-3">
                                    ...
                                    <li><strong>Case Number:</strong> ${case_number}</li>
                                    <li><strong>Process:</strong> ${business_process_sub_name || 'Not specified'}</li>
                                    <li><strong>Type:</strong> Search Report</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    <p class="text-muted small mt-3">
                        This will generate a search report template based on the case details.
                        Existing content in the editor will be replaced.
                    </p>
                </div>`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: '<i class="fas fa-file-signature me-1"></i>Compose Search Report',
            cancelButtonText: '<i class="fas fa-times me-1"></i>Cancel',
            confirmButtonColor: '#0d6efd',
            cancelButtonColor: '#6c757d',
            width: 500,
                    ...
                    error: function(xhr, status, error) {
                        ...
                        Swal.fire({
                            ...
                            html: `<div class="text-center">
                                    ...
                                    <h5 class="mb-2">Unable to Compose Search Report</h5>
                                    <p class="text-danger small">${error || 'Server error occurred'}</p>
                                    <div class="alert alert-warning mt-3">
                                        <i class="fas fa-lightbulb me-2"></i>
                                        Please try again or contact system administrator
    ...
    $(document).on('keydown', function(e) {
        ...
        if (e.ctrlKey && e.key === 's') {
            e.preventDefault();
            $('#lc_btn_save_search_report_2').click();
        }
    });

    // Optional: Add tooltip for keyboard shortcut
    $(document).ready(function() {
        const saveBtn = $('#lc_btn_save_search_report_2');
        if (saveBtn.length) {
            const originalTitle = saveBtn.attr('title') || 'Save Search Report';
            saveBtn.attr('data-bs-toggle', 'tooltip');
            saveBtn.attr('data-bs-placement', 'top');
    ...
    $('#btn_preview_search_report, #btn_preview_search_report_2, #btn_preview_search_report_3, #btn_preview_search_report_4').on('click', function(e) {
        ...
        $.ajax({
            ...
            success: function(pdfBlob) {
                // Create file object from blob
                const file = new File([pdfBlob], `Search_Report_${job_number}_${case_number}.pdf`, {
                    type: "application/pdf",
                    lastModified: Date.now()
                });
    ...
    $('#lc_btn_approve_search_for_signature').click(function(event) {
        event.preventDefault();
        ...
        Swal.fire({
            ...
            html: `<div class="text-center">
                    ...
                    <h5 class="mb-2">Secure Verification Required</h5>
                    <p class="text-muted">You are about to generate a one-time password for search report approval</p>
                        ...
                        <div class="d-flex">
                            ...
                            <div class="text-start">
                                ...
                                <ul class="mb-0 ps-3">
                                    ...
                                    <li><strong>Case:</strong> ${case_number}</li>
                                    <li><strong>Transaction:</strong> ${transaction_number}</li>
                                    <li><strong>Action:</strong> Search Report Approval</li>
                                    <!--<li><strong>Requested By:</strong> ${send_by_name || 'Current User'}</li>-->
                                </ul>
                            </div>
                        </div>
            ...
            if (result.isConfirmed) {
                ...
                $("#confirm_otp_for_approval_search").modal('hide');
                ...
                Swal.fire({
                    ...
                    html: `<div class="text-center">
                            ...
                            <h6 class="mb-2">Secure Authentication</h6>
                            <p class="text-muted">Enter the 6-digit verification code to approve the search report</p>
                        ...
                        $.ajax({
                            ...
                            success: function(verificationResponse) {
                                ...
                                Swal.fire({
                                    ...
                                    if (!isVerified && finalResult.dismiss === Swal.DismissReason.cancel) {
                                        // Regenerate OTP
                                        $('#lc_btn_approve_search_for_signature').click();
                                    }
                                });
                            },
                            error: function(xhr, status, error) {
    ...
    $(document).on('keydown', function(e) {
        ...
        if (e.ctrlKey && e.key === 's') {
            e.preventDefault();
            $('#lc_btn_save_search_report_3').click();
        }
    });
    ...
    $('#lc_btn_add_coordinate_2').on('click', function () {
        $(document).off('focusin.modal');
        // 🔥 WAIT FOR BOOTSTRAP TO RELEASE FOCUS
        setTimeout(openSwal_2, 300);
    });

    // Handle Add Coordinate button click
    function openSwal_2() {
        Swal.fire({
            target: document.getElementById('add_new_records_Info_frrv'),
            title: '<i class="bi bi-plus-circle text-primary me-2"></i>Add Coordinate',
            html: `<form id="coordinateForm">
                <div class="text-start">
                    <!-- Name Field -->
            ...
            preConfirm: () => {
                return new Promise((resolve, reject) => {
                    ...
                    if (!isValid) {
                        ...
                        resolve({
                            name: name || `Point ${$('#coordinatelis_Tabl_2 tbody tr').length + 1}`,
                            x: parseFloat(xCoord),
                            y: parseFloat(yCoord)
                        });
                    }
    ...
    function addCoordinateToTable(name, x, y) {
        ...
        // Remove empty state row if it exists
        const emptyRow = tbody.find('#noCoordinatesRow_2');
        if (emptyRow.length) {
            emptyRow.remove();
        }
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    row.fadeOut(300, function() {
                        ...
                        // If table is empty, show empty state
                        if ($('#coordinatelis_Table_2 tbody tr').length === 0) {
                            tbody.append(`
        ...
        // Update statistics
        updateCoordinateStats_2();
        
        // Optionally update WKT polygon if needed
        updateWktFromCoordinates_2();
    }

    // Function to update coordinate statistics
    function updateCoordinateStats_2() {
        const table = $('#coordinatelis_Table_2');
        const rows = table.find('tbody tr:not(#noCoordinatesRow_2)');
        const total = rows.length;
        
        // Update statistics display if it exists
        const totalEl = document.getElementById('totalCoordinates_2');
        if (totalEl) {
            totalEl.textContent = total;
        }
        
        // Update summary text if it exists
        const summaryEl = document.getElementById('coordinateSummary_2');
        if (summaryEl) {
            summaryEl.textContent = 
                total > 0 ? `${total} coordinate${total !== 1 ? 's' : ''} added` : 'No coordinates added';
        }
    ...
    // Function to update WKT polygon from coordinates
    function updateWktFromCoordinates_2() {
        const table = $('#coordinatelis_Table_2');
        const rows = table.find('tbody tr:not(#noCoordinatesRow_2)');
        ...
        if (rows.length >= 3) { // Need at least 3 points for a polygon
            ...
            const wkt = `POLYGON((${coordinates.join(', ')}))`;
            $('#lc_bl_wkt_polygon_2').val(wkt);
            
            // Update polygon status
            const statusEl = document.getElementById('polygonStatus_2');
            if (statusEl) {
                statusEl.textContent = 'Yes';
                statusEl.parentElement.parentElement.querySelector('.card')
                    .classList.add('border-success');
    ...
    // Update statistics when modal opens
    $('#add_new_records_Info_frrv').on('shown.bs.modal', function() {
        updateCoordinateStats_2();
    });
    ...
    $('#btn_lc_save_parcel_for_search').on('click', function(e) {
        ...
        var job_number = $("#cs_main_job_number").val();
        var case_number = $("#cs_main_case_number").val();
        var wkt_polygon = $("#lc_bl_wkt_polygon_2").val() || $("#lc_fr_bl_wkt_polygon_2").val();
        var send_by_id = localStorage.getItem('userid') || '';
        var send_by_name = localStorage.getItem('fullname') || 'System User';
            ...
            if (result.isConfirmed && result.value && result.value.success) {
                Swal.fire({
                    ...
                    willClose: () => {
                        ...
                        // Clear coordinates table if needed
                        const table = $('#coordinatelis_Table_2 tbody');
                        if (table.find('tr').length > 0 && !table.find('#noCoordinatesRow_2').length) {
                            Swal.fire({
                                title: 'Clear Coordinates?',
                                text: 'Do you want to clear the coordinate table?',
                                icon: 'question',
    ...
    function retrySaveParcel() {
        Swal.close();
        $('#btn_lc_save_parcel_for_search').click();
    }

    // Remove the entire modal HTML code and replace with this JavaScript

    // Handle CSV upload button click
    $('#lrd_btn_add_coordinate_by_csv_2').on('click', function() {
        // Create SweetAlert modal for file upload
        Swal.fire({
            title: 'Upload Coordinates CSV',
            html: `
            ...
            if (result.isConfirmed) {
                ...
                // Process the uploaded coordinates
                processUploadedCoordinates_2(coordinates);
    ...
    // Function to process uploaded coordinates
    function processUploadedCoordinates_2(coordinates) {
        // Clear existing table rows
        const table = $('#coordinatelis_Table_2');
        const tbody = table.find('tbody');
        ...
        // Optionally update map visualization
        if (typeof updateMapWithCoordinates === 'function') {
            updateMapWithCoordinates_2(coordinates);
        }
        ...
        setTimeout(() => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    // 🔵 Open Bootstrap modal
                    const modalEl = document.getElementById('add_new_records_Info_frrv');
                    const modal = bootstrap.Modal.getOrCreateInstance(modalEl);
                    modal.show();
                }
            });
    ...
    // Handle Clear All Coordinates button click
    $('#btn_clear_all_coordinates_2').on('click', function() {
        const table = $('#coordinatelis_Table_2');
        const tbody = table.find('tbody');
        const coordinateRows = tbody.find('tr[data-x][data-y]');
        const coordinateCount = coordinateRows.length;
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                coordinateRows.fadeOut(500, function() {
                    ...
                    // Update button state
                    $('#btn_clear_all_coordinates_2').prop('disabled', true);
                    
                    // Update other dependent button states
                    updateCoordinateDependentButtons_2();
    ...
    $(document).on('click', '#add_records_information_notes', function(e) {
        console.log("Add Records Information Notes clicked");
        $("#addNotesModal").modal('show');
        $('#addNotesModal #an_type').val('Regsitration_Search');
    });
    ...
    $('#form_add_notes').on('submit', function(e) {
        ...
        // Form validation
        var an_description = $("#an_description").val().trim();
        ...
        if (!an_description) {
            Swal.fire({
                icon: 'warning',
                title: 'Missing Information',
                text: 'Please enter a note description.',
                confirmButtonColor: '#3085d6',
                confirmButtonText: 'OK'
            });
            $("#an_description").focus();
            return false;
        }
        
        // Determine action text based on ID
        var actionText = an_id === '0' ? 'add' : 'update';
        var actionTitle = an_id === '0' ? 'Add New Note' : 'Update Note';
        var actionMessage = an_id === '0' 
        ...
        Swal.fire({
            ...
            html: actionMessage + 
                ...
                '<strong>Case:</strong> ' + an_case_number + '<br>' +
                '<strong>Job:</strong> ' + an_job_number + '<br>' +
                '<strong>Type:</strong> ' + an_type +
                '</div>',
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire({
                    ...
                    html: '<div class="text-center">' +
                        ...
                        '</div>' +
                        '<strong>Note has been ' + (an_id === '0' ? 'added' : 'updated') + ' successfully!</strong><br>' +
                    ...
                    $(document).trigger('notes:updated', {
                        action: actionText,
                        caseNumber: an_case_number,
                        jobNumber: an_job_number,
                        id: result.value || an_id
                    });
                });
            }
        });
