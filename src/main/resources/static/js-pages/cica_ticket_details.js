   ...
   $("#update_status_form").on('submit', function(e) {
    ...
    if (!status_note) {
        Swal.fire({
            icon: 'warning',
            title: 'Input Required',
            text: 'Please enter a status note',
            timer: 2000,
            showConfirmButton: false
        });
        return false;
    ...
    if (!description) {
        Swal.fire({
            icon: 'warning',
            title: 'Input Required',
            text: 'Please enter a reply description',
            timer: 2000,
            showConfirmButton: false
        });
        return false;
    ...
    if (isChecked == true) {
        // Check if file is selected when checkbox is checked
        if (jQuery('#file')[0].files.length === 0) {
            Swal.fire({
                icon: 'warning',
                title: 'File Required',
                text: 'Please select a file to attach',
                timer: 2000,
                showConfirmButton: false
            });
            return false;
        ...
        success: function(response) {
            ...
            if (json_result.success) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Error occurred uploading file',
                    timer: 2000,
                    showConfirmButton: false
                });
            }
