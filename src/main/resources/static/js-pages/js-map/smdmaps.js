					...
					document
							...
							.addEventListener(
									...
									function() {
										...
										$('#smd_bl_wkt_polygon').val(wkt);

										if(wkt) {
											$('#smd_btn_request_add_existing_parcel').removeClass('d-none')
										}
										...
										try {
											...
											view.fit(smd_lc_searchLayer.getSource().getExtent());
											smd_map.getView().fit(smd_lc_searchLayer.getSource().getExtent(), {
												size: smd_map.getSize(),
												maxZoom: 16
											});
										} catch (error) {
...
$(document).off('click.smdAddExistingParcel', '#smd_btn_request_add_existing_parcel')
	.on('click.smdAddExistingParcel', '#smd_btn_request_add_existing_parcel', function(e) {
	e.preventDefault();

	var smd_bl_wkt_polygon = $.trim($('#smd_bl_wkt_polygon').val());

	if (!smd_bl_wkt_polygon) {
		Swal.fire({
	...
	Swal.fire({
		...
		html: `
			...
			<div class="form-group text-start mb-2">
				<label for="smd_bl_add_exist_reference_number" class="form-label">Reference Number <span class="text-danger">*</span></label>
				<input type="text" id="smd_bl_add_exist_reference_number" class="form-control" placeholder="Enter reference number">
			</div>
            <div class="form-group text-start mb-2">
				<label for="smd_bl_add_exist_file_number" class="form-label">File Number</label>
				<input type="text" id="smd_bl_add_exist_file_number" class="form-control" placeholder="Enter file number">
			</div>
            <div class="form-group text-start mb-2">
				<label for="smd_bl_add_exist_party_1" class="form-label">Party 1 (Grantor)</label>
				<textarea id="smd_bl_add_exist_party_1" class="form-control" placeholder="Enter party"></textarea>
			</div>
            <div class="form-group text-start mb-2">
				<label for="smd_bl_add_exist_party_2" class="form-label">Party 2 (Grantee)</label>
				<textarea id="smd_bl_add_exist_party_2" class="form-control" placeholder="Enter party"></textarea>
			</div>
		            <div class="form-group text-start mb-2">
				<label for="smd_bl_add_exist_acreage" class="form-label">Acreage <span class="text-danger">*</span></label>
				<input type="number" id="smd_bl_add_exist_acreage" min="0.01" class="form-control" placeholder="Enter acreage">
			</div>
			<div class="form-group text-start mb-2">
				<label for="smd_bl_add_exist_locality" class="form-label">Locality <span class="text-danger">*</span></label>
				<input type="text" id="smd_bl_add_exist_locality" class="form-control" placeholder="Enter locality">
			</div>
		            <div class="form-group text-start mb-2">
				<label for="smd_bl_add_exist_comments" class="form-label">Comments<span class="text-danger">*</span></label>
				<textarea id="smd_bl_add_exist_comments" class="form-control" placeholder="Enter comments"></textarea>
			</div>
		`,
		showCancelButton: true,
		...
		focusConfirm: false,
		preConfirm: function() {
			var referenceNumber = $.trim($('#smd_bl_add_exist_reference_number').val());
			...
			var locality = $.trim($('#smd_bl_add_exist_locality').val());
			...
			var acreage = $.trim($('#smd_bl_add_exist_acreage').val());
			...
			var comments = $.trim($('#smd_bl_add_exist_comments').val());

            if (!comments) {
				Swal.showValidationMessage('Comments is required');
				return false;
			}
			var fileNumber = $.trim($('#smd_bl_add_exist_file_number').val());
			var party1 = $.trim($('#smd_bl_add_exist_party_1').val());
			var party2 = $.trim($('#smd_bl_add_exist_party_2').val());
		...
		$.ajax({
			...
			data: {
				...
				wkt_polygon: smd_bl_wkt_polygon,
			...
			success: function(response) {
				...
				var json_p;
				try {
					json_p = typeof response === 'string' ? JSON.parse(response) : response;
				} catch (parseError) {
					Swal.fire({
						title: 'Request Failed',
				...
				if (json_p && json_p.success === true) {
					initiateReqAddExistingParcel(json_p.glpin, json_p.reference_number, json_p.party_1, json_p.party_2);
					return;
				}

				Swal.fire({
					title: 'Request Failed',
					text: (json_p && (json_p.message || json_p.msg)) || 'Unable to submit the parcel request.',
					icon: 'error',
					confirmButtonText: 'OK'
				});
...
window.initiateReqAddExistingParcel = function(parcelId, referenceNumber, party1, party2) {
	...
	// Close the underlying modal
    $('#smdparcelIndormation').modal('hide');
