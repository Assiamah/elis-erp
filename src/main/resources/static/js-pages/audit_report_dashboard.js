...
// Create January 1st of the current year
...
const dateFromPicker = flatpickr("#datefrom", {
    ...
    altInput: true,
    altFormat: "j F Y",             // Display: 1 January 2025
    allowInput: true,
    defaultDate: firstDayOfYear,   // ← This sets it to Jan 1 of current year
    onReady: function(selectedDates, dateStr, instance) {
        // On load, also populate the hidden fields with the default date
        if (selectedDates.length > 0) {
            let formattedDisplay = instance.formatDate(selectedDates[0], "j F Y");
            $('#start_date').val(dateStr);           // YYYY-MM-DD
            $('#startdate').val(formattedDisplay);   // 1 January 2025
            startDate = dateStr;

            // Set minDate for dateTo picker right away
    ...
    onChange: function(selectedDates, dateStr, instance) {
        if (selectedDates.length > 0) {
            ...
            $('#startdate').val(formattedDisplay);

            // Update the "Date To" picker's minDate
            dateToPicker.set('minDate', dateStr);
